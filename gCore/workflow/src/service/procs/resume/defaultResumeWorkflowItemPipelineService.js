...
module.exports = {
    ...
    validateRequest: function (request, response, process) {
        this.LOG.debug('Validating request to resume items');
        if (!request.tenant) {
    ...
    buildQuery: function (request, response, process) {
        this.LOG.debug('Building query to resume items');
        process.nextSuccess(request, response);
    },
    prepareResponse: function (request, response, process) {
        this.LOG.debug('Preparing response for resume items');
        process.nextSuccess(request, response);
    },
    preResumeInterceptors: function (request, response, process) {
        ...
        if (interceptors && interceptors.preResume) {
            this.LOG.debug('Applying preResume interceptors for workflow item pause');
    ...
    preResumeValidators: function (request, response, process) {
        ...
        if (validators && validators.preResume) {
            this.LOG.debug('Applying preResume validators for workflow item pause');
