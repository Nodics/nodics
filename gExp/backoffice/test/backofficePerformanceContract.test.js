...
global.CONFIG = {
    get: (key) => (key === 'backofficeRegistry' ? properties : key === 'defaultTenant' ? 'default' : undefined),
};
...
const registry = Object.assign({}, registryDefinition, {
    _store: store,
    _metrics: {
        registrations: 0,
        renewals: 0,
...
async function run() {
    ...
    let registered = await registry.registerBatch(
        {
            instanceId: 'runtime-benchmark',
            environment: 'benchmark',
            server: 'benchmark-server',
            registrations: modules.map((moduleName) => ({
                moduleName,
                displayName: moduleName,
                canonicalIdentity: moduleName,
                instanceId: 'runtime-benchmark',
    ...
    let inventory = await registry.adminList({
        tenant: 'default',
        authData: {
            tokenType: 'access',
        ...
        query: {
            environment: 'benchmark',
            limit: String(budget.maxAdminResultPage),
        },
    });
    ...
    global.SERVICE = {
        DefaultBackofficeDiscoveryService: {
            buildObservedUrl: () => 'https://module.example/ready',
        },
        DefaultModuleService: {
            buildExternalRequest: (options) => options,
            fetch: () =>
                new Promise((resolve) => {
                    fetches++;
                    activeFetches++;
    ...
    await Promise.all(
        Array.from({ length: budget.hostedModulesPerInstance }, (_, index) =>
            availability.scheduleObservation({
                moduleName: 'hosted' + index,
                instanceId: 'shared-runtime',
                clientCallable: true,
    ...
    const security = Object.assign({}, securityDefinition, {
        _counters: new Map(),
        _inflight: new Map(),
        _results: new Map(),
    });
    ...
    await Promise.all(
        Array.from({ length: budget.concurrentRefreshRequests }, () =>
            security.executeRefresh(refreshRequest, 'cms', async () => {
                refreshExecutions++;
    ...
    console.log(
        'BackOffice performance evidence validated: registrationModules=' +
            budget.registrationModules +
            ', registryLeases=' +
            budget.registryLeases +
            ', hostedModules=' +
            budget.hostedModulesPerInstance +
            ', registrationMs=' +
            registrationMs.toFixed(3) +
            ', adminListMs=' +
            adminMs.toFixed(3) +
            ', adminStoreScans=' +
            scans +
