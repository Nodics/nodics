...
Start with the beginner and developer guide (canonical documentation: `capability.commerce.technical-reference`). The public resolver is `GET /context/resolve`; protected human management is under `/management/:resource` for `storefronts` and `endpoints`.
