...
| --- | --- | --- | --- | --- |
| Project root | Confirm project name, `groupName`, project module catalog, environment catalog, ownership summary, and whether project composition tests are required. | Root files, `config/`, `llm/`, `modules/`, `envs/`, and optional project composition `test/`. | Empty `src/`, empty `data/`, module `docs/`, server/node generated reports, environment/server/node files before topology approval. | `npm run test:module-metadata`, `npm run test:topology` when topology exists, `npm run llm:generate`, `npm run llm:validate`. |
| Project module group | Confirm child module names, ordering/index values, shared defaults, and whether the group owns executable behavior. | Root files, `config/`, `llm/`, child module folders, and optional `src/`, `data/`, `test/` only for owned behavior. | Empty data/source/test folders when the group only aggregates children; module `docs/`. | `npm run test:module-metadata`, `npm run docs:coverage:source -- --limit=20`, `npm run llm:generate`, `npm run llm:validate`. |
| Framework group module | Confirm framework child modules, ordering/index values, shared framework defaults, and no customer/project-specific behavior. | Root files, `config/`, `llm/`, child module folders, and optional shared source/data/tests only when framework-owned. | Customer topology, project `groupName`, project `envs/`, project module catalogs, module `docs/`. | `npm run test:module-metadata`, `npm run test:tooling`, `npm run docs:coverage:source -- --limit=20`, `npm run llm:validate`. |
| Capability module | Confirm capability contract, owner layer, source surfaces, data ownership, override path, provider neutrality, and tests for default plus override/customization behavior. | Root files, `config/`, `llm/`, owned `src/` directories, owned `data/`, owned `test/`, and generated artifacts only from source definitions. | Environment/server/node topology folders, hardcoded provider selection, copied framework files for one-method changes, behavior outside loader-visible paths, module `docs/`. | `npm run test:module-metadata`, targeted module tests, `npm run docs:coverage:source -- --limit=20`, `npm run llm:generate`, `npm run llm:validate`. |
| Provider module | Confirm provided capability, provider-specific config keys, selection mechanism, credential boundary, and fallback/default behavior. | Capability-style root files plus provider-owned `src/`, config defaults, tests, canonical documentation, and LLM guidance. | Global project/customer provider selection, credentials in source, unrelated provider behavior, module `docs/`. | Targeted provider tests, integration governance checks when available, `npm run test:module-metadata`, `npm run llm:validate`. |
| Environment module | Confirm environment purpose, deployment-wide defaults, server catalog, environment-owned data, and topology tests. | Root files, `config/`, `llm/`, server module folders, optional environment `data/` and `test/`. | Per-process `activeModules`, node ids, reusable capability implementation, empty `src/` unless real environment override is approved, module `docs/`. | `npm run test:topology`, `npm run test:topology:consolidated`, `npm run test:topology:modular` when applicable, `npm run llm:validate`. |
| Server module | Confirm process role, local active groups/modules, runtime flags, endpoint coordinates, data ownership, node list, and startup/API/internal-communication tests. | Root files, `config/`, `llm/`, server-owned `src/`, `data/`, `test/`, `generated/`, and child node modules when approved. | Project `modules/`, project `envs/`, environment-wide policy, node-local ids in server-independent files, module `docs/`. | `npm run test:topology`, targeted startup/API tests, targeted internal communication tests, `npm run docs:coverage:source -- --limit=20`, `npm run llm:validate`. |
| Node module | Confirm parent server, node id, node-specific overrides, diagnostics/report ownership, and node validation tests. | Root files, `config/`, `llm/`, optional node-owned `src/`, `data/`, `test/`, and `generated/`. | Server-wide `activeModules`, environment-wide data, child server/env folders, reusable capability implementation, module `docs/`. | Targeted node/topology tests, `npm run test:topology` when topology changes, `npm run llm:validate`. |
...
| --- | --- | --- | --- | --- |
| Project root | `package.json`, `nodics.js`, `AGENTS.md`, `README.md` | `config/`, `llm/`, `modules/`, `envs/` | `test/` for project composition contracts only | `src/`, `data/`, module `docs/`, server/node `generated/` |
| Project module group | `package.json`, `nodics.js`, `AGENTS.md`, `README.md` | `config/`, `llm/` | child module folders, `src/`, `data/`, `test/` only when the group owns behavior/data/tests | `data/` when the group only aggregates children, module `docs/` |
| Framework group module | `package.json`, `nodics.js`, `AGENTS.md`, `README.md` | `config/`, `llm/` | child module folders, `src/`, `data/`, `test/` only when the group owns shared behavior/data/tests | customer/project folders, project-specific `envs/`, module `docs/` |
| Capability module | `package.json`, `nodics.js`, `AGENTS.md`, `README.md` | `config/`, `llm/` | `src/`, `data/`, `test/`, `generated/` when the capability owns source, records, tests, or generated runtime artifacts | topology folders such as `envs/`, server/node folders, module `docs/` |
| Environment module | `package.json`, `nodics.js`, `AGENTS.md`, `README.md` | `config/`, `llm/` | server module folders, `data/`, `test/` for environment-owned data/topology validation | `src/` unless the environment owns real source overrides, module `docs/` |
| Server module | `package.json`, `nodics.js`, `AGENTS.md`, `README.md` | `config/`, `llm/` | node module folders, `src/`, `data/`, `test/`, `generated/` for process-owned overrides, data, tests, and runtime reports | project `modules/`, project `envs/`, module `docs/` |
| Node module | `package.json`, `nodics.js`, `AGENTS.md`, `README.md` | `config/`, `llm/` | `src/`, `data/`, `test/`, `generated/` for node-specific overrides, records, diagnostics, or reports | child environment/server folders, module `docs/` |
