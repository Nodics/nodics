...
function getActiveOutputModule() {
    let moduleName = (NODICS.getNodeName && NODICS.getNodeName()) ||
    ...
    if (!moduleObject || !moduleObject.path) {
        throw new Error('Active server or node module is required to generate governance report');
    }
    return {
        name: moduleName,
...
async function run() {
    ...
    let outputPath = path.join(outputDirectory, activeOutputModule.name + '.governance-report.json');
