...
const config = require('../gFramework/nConfig');
...
function getDefaultOutputDir() {
    const modulePath = NODICS.getNodePath() || NODICS.getServerPath();
    if (!modulePath) {
        throw new CLASSES.NodicsError('Unable to resolve active server or node path for generated OpenAPI contract output');
    }
...
function assertOutputDirAllowed(outputDir) {
    ...
    if (resolvedOutputDir === docsDir || resolvedOutputDir.indexOf(docsDir + path.sep) === 0) {
        throw new CLASSES.NodicsError('Generated API contracts must be written under the active server or node module, not under docs');
    }
}
