...
/**
 * @module nTooling/mcp/readOnlyGovernanceService
 ...
 */
function toRelative(home, filePath) {
    return toPosix(path.relative(home, filePath));
}
 ...
 */
function resolveInsideHome(home, inputPath) {
 ...
 */
function discoverModules(home, directory, modules = []) {
    if (!fs.existsSync(directory)) {
        return modules;
    }
    if (directory !== home && isModuleDirectory(directory)) {
        const packageJson = readJsonIfExists(path.join(directory, 'package.json')) || {};
        ...
        modules.push({
            ...
            path: directory,
            relativePath: toRelative(home, directory),
            owns: nodics.owns || [],
    ...
    fs.readdirSync(directory, { withFileTypes: true })
        ...
        .forEach(entry => discoverModules(home, path.join(directory, entry.name), modules));
    return modules;
}
 ...
 */
function summarizeWorkspace(home, modules) {
    const byKind = {};
 ...
 */
function findOwningModule(modules, targetPath) {
 ...
 */
function findAgentsChain(home, targetPath) {
    ...
    while (true) {
        ...
        if (fs.existsSync(agentsPath)) {
            agents.push({
                path: toRelative(home, agentsPath),
                directory: toRelative(home, cursor),
 ...
 */
function getGeneratedContext(home, moduleObject) {
    if (!moduleObject) {
        return null;
    }
    ...
    return {
        ...
        contextPath: fs.existsSync(contextPath) ? toRelative(home, contextPath) : null,
        manifestPath: manifest ? toRelative(home, manifestPath) : null,
 ...
 */
function buildChangeImpact(home, modules, paths) {
    return (paths || []).map(inputPath => {
        const absolutePath = resolveInsideHome(home, inputPath);
        const moduleObject = findOwningModule(modules, absolutePath);
        const relativePath = toRelative(home, absolutePath);
        return {
            path: relativePath,
            owningModule: moduleObject && moduleObject.name,
            owningModulePath: moduleObject && moduleObject.relativePath,
            artifactTypes: classifyArtifact(relativePath),
            nearestAgents: findAgentsChain(home, absolutePath),
            generatedContext: getGeneratedContext(home, moduleObject),
            guidance: [
...
function createReport(options = {}) {
    ...
    const modules = discoverModules(home, home, [])
        .sort((left, right) => left.relativePath.localeCompare(right.relativePath));
    const targetPath = resolveInsideHome(home, options.path || '.');
    const owningModule = findOwningModule(modules, targetPath);
    return {
        ...
        workspace: summarizeWorkspace(home, modules),
        ...
        lookup: {
            path: toRelative(home, targetPath),
            owningModule: owningModule && owningModule.name,
            owningModulePath: owningModule && owningModule.relativePath,
            nearestAgents: findAgentsChain(home, targetPath),
            generatedContext: getGeneratedContext(home, owningModule)
        },
        changeImpact: buildChangeImpact(home, modules, options.changePaths || [])
    };
}
