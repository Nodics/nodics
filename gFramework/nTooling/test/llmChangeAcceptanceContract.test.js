...
requireClauses('gSetup/llm/contracts/developer-implementation-contract.md', [
    ...
    'loader-managed path and canonical file shape',
    'must export',
...
requireClauses('gSetup/llm/standards/nodics-structure-matrix.md', [
    '# Nodics Structure Matrix',
    '| Project root | `project` |',
    '| Environment module | `group` |',
    '| Server module | `server` |',
    '| Node module | `node` |',
    '`package.json.nodics.kind` classifies the boundary',
    '`activeModules` loads behavior into the current process',
    '## Metadata Matrix',
    '`package.json` is the source of truth for a boundary identity',
    '`groupName` groups company/customer projects',
    '`runtimeModule`',
    ...
    '| Provider module | Provider-specific defaults',
    ...
    'Tenant/customer context may refine behavior after process startup',
    'Only after the hierarchy is approved should files be generated',
    '## Generation Checklist Matrix',
    'Use this checklist after the hierarchy proposal is approved',
    '| Provider module | Confirm provided capability',
    'Generated structure is not',
    'complete until metadata, folders, configuration',
    'Keep `package.json.nodics.owns` aligned with the generated folders',
    'Put configurable values in `config/properties.js`',
    'Add or update override/customization tests for every new or changed',
    'Regenerate generated artifacts from source definitions and regenerate LLM',
    '## Folder Matrix',
    'Required root files',
    'Do not generate empty',
    'Project root | `package.json`, `nodics.js`, `AGENTS.md`, `README.md`',
    '## Source Folder Matrix',
    '`src/router/`',
    '`routers.js`, `appConfig.js`',
    '`src/service/`',
    '`defaultSampleService.js`',
    '## Data Folder Matrix',
    '`data/init/`',
    '`data/core/`',
    '`data/sample/`',
    ...
    'Import run history and diagnostics',
    'Tenant/customer runtime overrides',
    'Versioned publishable data',
    'Generated artifacts must never become hidden authority',
    ...
    'who creates it, who loads it, who validates it',
    '## Generator Alignment Rule',
    'Nodics structure generation must use `structure:generate`',
    '`generate:provider` must produce scaffolds',
    'Generators must not copy retired `nCommon/templates`',
    '## Topology Planning Rule',
    'Full project topology generation must be plan-first',
    '`npm run structure:plan` to print the proposed project',
    'Use `--apply` only after the hierarchy is approved',
    'must not silently create modules',
    '## Compliance Audit Rule',
    'Run `npm run structure:audit`',
    'retired `router.js` and `pipelinesDefinition.js` files',
