...
| `gFramework/nTooling/test/dependencyOwnershipContract.test.js` | `test` | `documented` | 0/0 | Verifies package dependency ownership so external providers, runtime frameworks, and shared utilities remain installed only by the root package while root dependency governance records the owning Nodics modules. |  |
