...
| `gFramework/nService/src/lib/modulesConfigurationContainer.js` | `src` | `documented` | 0/0 | Runtime container for module topology configuration loaded from layered `servers` properties. It builds `ModuleConfiguration` objects used for internal module communication and distributed node routing. |  |
...
| `gFramework/nService/test/moduleRegistrationAgent.test.js` | `test` | `documented` | 0/0 | Validates non-blocking lifecycle registration, all-active-module batching, service-token use, adaptive retry, and graceful deregistration. |  |
