...
A project may enable Hazelcast only by providing a real layered connection service and cache service, setting `enabled: true`, and declaring truthful capabilities for distribution, atomic consume, TTL, serialization, and invalidation. The normal cache engine validator then qualifies that implementation before activation.
