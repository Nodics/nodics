...
Set engine options such as `url`, socket/TLS, database, username, and password through layered configuration. Run `NODICS_CACHE_REDIS_URL=... npm run test:suite -- --suite=cache-redis-live` against an isolated Redis endpoint for guarded integration qualification.
