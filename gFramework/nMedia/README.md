...
## Media Format Policy Management

Media format policy follows the same configuration-first rule as folder
policy. Effective upload and variant eligibility is read from `media.formats`;
the generated `mediaFormat` model is useful for discovery, audit, seed data,
and BackOffice workbench flows, but the live authority remains the nMedia
policy service unless a deployment adds an explicit synchronization workflow.

nMedia exposes secured format-policy operations:

| Operation  | Route                                                          | Permission                   | Purpose                                                            |
| ---------- | -------------------------------------------------------------- | ---------------------------- | ------------------------------------------------------------------ |
| Create     | `PUT /nodics/media/v0/formats/policy`                          | `media.format.policy.manage` | Add one effective format policy.                                   |
| Update     | `PATCH /nodics/media/v0/formats/policy/{formatCode}`           | `media.format.policy.manage` | Change bounded mutable format fields.                              |
| Activate   | `POST /nodics/media/v0/formats/policy/{formatCode}/activate`   | `media.format.policy.manage` | Re-enable future upload/variant use for a format.                  |
| Deactivate | `POST /nodics/media/v0/formats/policy/{formatCode}/deactivate` | `media.format.policy.manage` | Block future upload/variant use without deleting historical media. |

The mutable format fields are `name`, `description`, `purpose`, `width`,
`height`, `formatFamily`, and `status`. `formatFamily` is intentionally generic
(`ORIGINAL`, `RESPONSIVE`, `PREVIEW`, `IMPORT`, `EXPORT`, `DOCUMENT`, or
`CUSTOM`) so Product, CMS, import, export, and customer modules can reuse the
same format vocabulary without leaking business ownership into nMedia. Inactive
formats remain visible as configuration but are rejected by upload validation.
...
## Media Source Context Metadata

Media source context metadata is the backend-owned bridge between business
source types and technical folder/format policy. Axis and other clients should
use this contract to decide which upload choices to display instead of
hardcoding source-type mappings in the browser.

The default contexts live under `media.contexts` in
`gFramework/nMedia/config/properties.js`:

| Context        | Default folder  | Default format | Manual upload | Purpose                                         |
| -------------- | --------------- | -------------- | ------------- | ----------------------------------------------- |
| `dataImports`  | `importSources` | `importFile`   | Yes           | Files staged for governed `nImport` processing. |
| `dataExports`  | `exportFiles`   | `exportFile`   | No            | Files generated by `nExport` workflows.         |
| `productMedia` | `productAssets` | `original`     | Yes           | Product/catalog assets.                         |
| `contentMedia` | `cmsAssets`     | `original`     | Yes           | CMS and storefront content assets.              |
| `utilityMedia` | `default`       | `original`     | Yes           | General governed files.                         |

Clients can read the safe context projection through:
...
module can add a new business source type, disable manual upload, or remap a
context to a different folder/format without changing Axis or provider code.
...
nMedia owns reusable set-entry operations for the logical set itself:

| Operation       | Route                                                                   | Permission         | Purpose                                                                                                            |
| --------------- | ----------------------------------------------------------------------- | ------------------ | ------------------------------------------------------------------------------------------------------------------ |
| Add entry       | `POST /nodics/media/v0/sets/{mediaSetCode}/entries`                     | `media.set.manage` | Add a media item variant to a set.                                                                                 |
| Update entry    | `PATCH /nodics/media/v0/sets/{mediaSetCode}/entries/{entryCode}`        | `media.set.manage` | Update format, role, locale, channel, device, breakpoint, fallback, dimensions, position, primary flag, or status. |
| Remove entry    | `DELETE /nodics/media/v0/sets/{mediaSetCode}/entries/{entryCode}`       | `media.set.manage` | Remove a media item from the set without mutating Product/CMS owners.                                              |
...
These fields describe reusable media behavior only. Product/CMS usage,
placement, merchandising, publishing, storefront visibility, and content
lifecycle remain in the owning module and are represented in nMedia only by
`mediaReference` trace records where needed.
