...
/**
 * @module nAuth/test/integration/AuthRedisLiveIntegration
...
async function run() {
    if (!configuration.redisUrl) {
        console.log('nAuth distributed authentication live Redis NOT EXECUTED: set NODICS_AUTH_REDIS_LIVE_URL; use test:auth:redis:release to require it');
        return;
    }
    ...
    try {
        ...
        console.log('nAuth distributed authentication live Redis exactly-once contract validated');
    } finally {
