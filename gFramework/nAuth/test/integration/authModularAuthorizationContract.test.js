...
/**
 * @module nAuth/test/integration/AuthModularAuthorizationContract
...
const secret = 'auth-distributed-modular-jwt-secret-with-more-than-thirty-two-characters';
const values = {
    profileModuleName: 'profile',
    defaultTenant: configuration.tenant,
    defaultAuthDetail: { apiKey: 'auth-distributed-bootstrap-key-12345678901234567890', entCode: 'enterprise-a' },
    authSecurity: {
...
function runWorker(input) {
    return new Promise((resolve, reject) => {
        let worker = path.join(__dirname, 'authServiceTokenWorker.js');
...
async function run() {
    ...
    console.log('nAuth distributed authentication modular authorization contract validated');
}
