...
SERVICE.DefaultPointOfServiceService = SERVICE.DefaultStoreService;
...
(async () => {
  ...
  assert.strictEqual(
    routers.management.list.permission,
    "store.backoffice.read",
  );
  assert.strictEqual(
    routers.management.create.permission,
    "store.backoffice.manage",
  );
  assert.deepStrictEqual(routers.reference.resolve.authTokenTypes, ["service"]);
  await assert.rejects(
    management.list({
      authData: { tokenType: "service", enterprise: { code: "enterpriseA" } },
      params: { resource: "stores" },
      query: {},
    }),
    (error) => error.code === "ERR_STORE_00012",
  );
  await assert.rejects(
    management.list({
      authData: human,
  ...
  let list = await management.list({
    tenant: "tenantA",
    authData: human,
    params: { resource: "stores" },
    query: { limit: 10 },
  ...
  let created = await management.create({
    tenant: "tenantA",
    authData: human,
    params: { resource: "stores" },
    body: {
      code: "forged",
      enterpriseCode: "enterpriseB",
      storeCode: "apparel",
      name: "Apparel",
    },
  });
  ...
  let posList = await management.list({
    tenant: "tenantA",
    authData: human,
    params: { resource: "pointsOfService" },
    query: { storeCode: "electronics", status: "ACTIVE" },
  });
  assert.strictEqual(posList.resource, "pointsOfService");
  assert.strictEqual(calls[2].input.query.enterpriseCode, "enterpriseA");
  assert.strictEqual(calls[2].input.query.storeCode, "electronics");
  let posCreated = await management.create({
    tenant: "tenantA",
    authData: human,
    params: { resource: "pointsOfService" },
    body: {
  ...
  assert.strictEqual(posCreated.operation, "CREATED");
  assert.strictEqual(calls[3].input.model.enterpriseCode, undefined);
  assert.strictEqual(calls[3].input.model.code, undefined);
  let retired = await management.retire({
    tenant: "tenantA",
    authData: human,
    params: { resource: "stores", businessCode: "electronics" },
    body: {},
  });
  assert.strictEqual(retired.operation, "RETIRED");
  assert.deepStrictEqual(calls[4].input.query, {
    enterpriseCode: "enterpriseA",
    storeCode: "electronics",
  });
  assert.deepStrictEqual(calls[4].input.model, { status: "RETIRED" });
