...
`STOCK`, `PRE_ORDER`, and `BACKORDER` are finite/counter-managed capacity
patterns. `PERPETUAL` is unbounded and counterless. `DIGITAL`, `DROP_SHIP`, and
`MADE_TO_ORDER` are on-demand and counterless, but they can still require
provider or provisioning policy evidence.
...
## 6. Infinite and on-demand inventory examples

Not every sellable item has a finite warehouse counter.

For a subscription or digital entitlement that is always sellable, use a
counterless perpetual promise:
...
For an eSIM that is generated by an external provider after checkout, use an
on-demand digital promise:
...
Checkout can still reserve this promise so the cart/order has clear demand
evidence. Fulfillment or Workflow later calls the provider and attaches the real
provisioning evidence. If the eSIM profiles are pregenerated instead, model the
available quantity through Stock and track each profile through
`serializedStockUnit`.
...
If `SUBSCRIPTION_ACCESS` needs behavior beyond configured capacity policy, the
customer module should replace or decorate the promise policy/orchestration
service. It should not change framework source directly.
