...
- Public Storefront Availability must introspect an opaque handle for the `inventory` audience, replace caller sourcing scope, delegate to the existing Availability chain, and return only customer-safe totals. Never expose Pool, Warehouse, Balance, revision, or enterprise evidence publicly.
