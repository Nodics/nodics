...
assert(
  properties.order.checkoutEntry.policy.immutableFields.includes("orderCode"),
);
assert(
  properties.order.checkoutEntry.policy.immutableFields.includes("totalPrice"),
);
assert(
  properties.order.checkoutEntry.policy.immutableFields.includes(
    "lineGrossAmount",
  ),
);
assert.strictEqual(
  interceptors.orderEntryPreSavePolicy.handler,
  "DefaultOrderEntryPolicyService.prepareEntry",
);
assert.strictEqual(
  interceptors.orderEntryPreUpdatePolicy.handler,
  "DefaultOrderEntryPolicyService.prepareEntryUpdate",
);
assert.strictEqual(
  interceptors.orderEntryPreRemovePolicy.handler,
  "DefaultOrderEntryPolicyService.rejectHardDelete",
);
...
assert.strictEqual(
  orderEntry.indexes.individual.entryCode.options.unique,
  true,
);
...
const convertedEntry = policy.buildOrderEntryFromCartEntry(
  cartEntryPayload,
  { orderCode: "order-1" },
  {},
);
...
assert.strictEqual(convertedEntry.lineNetAmount, "19.04");
assert.strictEqual(convertedEntry.lineGrossAmount, "20.00");
assert.strictEqual(convertedEntry.taxTotal, "0.96");
assert.strictEqual(convertedEntry.taxInclusionMode, "TAX_INCLUSIVE");
assert.strictEqual(convertedEntry.taxIncluded, true);
assert.strictEqual(convertedEntry.taxQuoteLineCode, "tax-line-1");
assert.strictEqual(
...
assert.deepStrictEqual(
  policy.validateUpdate(
    convertedEntry,
    { lineGrossAmount: "19.99" },
    {
      immutableFields: ["lineGrossAmount"],
      statuses: ["ORDERED", "ALLOCATED"],
      allowedTransitions: { ORDERED: ["ALLOCATED"] },
    },
  ),
  ["lineGrossAmount is immutable"],
...
const serviceConvertedEntry = orderEntryPolicyService.buildFromCartEntry(
  cartEntryPayload,
  { orderCode: "order-2" },
);
global.SERVICE = {
...
assert.strictEqual(serviceConvertedEntry.taxQuoteCode, "tax-quote-1");
assert.throws(
  () =>
    orderEntryPolicyService.validateUpdate(serviceConvertedEntry, {
      lineGrossAmount: "10.00",
    }),
  /lineGrossAmount is immutable/,
);
(async () => {
  await orderEntryPolicyService.prepareEntryUpdate({
    query: { code: "entry-1" },
