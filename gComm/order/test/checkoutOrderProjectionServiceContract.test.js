...
global.SERVICE = {
    ...
    DefaultOrderService: {
        get: async (request) => ({ result: savedOrders.filter((order) => order.code === request.query.code) }),
        save: async (request) => {
...
const request = {
    ...
    cartCode: 'cart-1',
    entCode: 'enterpriseA',
    cart: { code: 'cart-1', entCode: 'enterpriseA', active: true, status: 'READY_FOR_CHECKOUT', currencyCode: 'USD' },
    cartEntries: [
        { cartCode: 'cart-1', entCode: 'enterpriseA', entryCode: 'entry-1', lineNumber: 1, catalogCode: 'catalogA', itemType: 'SKU', itemCode: 'phone', quantity: '3', unitCode: 'EA', currencyCode: 'USD', unitPrice: '10.00', totalPrice: '30.00', status: 'ACTIVE' },
        { cartCode: 'cart-1', entCode: 'enterpriseA', entryCode: 'entry-2', lineNumber: 2, catalogCode: 'catalogA', itemType: 'SKU', itemCode: 'case', quantity: '1', unitCode: 'EA', currencyCode: 'USD', unitPrice: '5.00', totalPrice: '5.00', status: 'ACTIVE' },
    ],
};

(async () => {
    assert.strictEqual(orderProperties.order.checkoutPlacement.orderProjection.orderStatus, 'PLACED');
    assert.strictEqual(orderProperties.order.checkoutPlacement.orderProjection.entryStatus, 'ORDERED');
    assert.strictEqual(orderSchemas.order.definition.cartCode.required, false);
    assert.strictEqual(orderSchemas.order.definition.workflowCarrierCode.required, false);
    assert.strictEqual(orderSchemas.order.indexes.common.cartCode.enabled, true);

    const projection = await projectionService.create(JSON.parse(JSON.stringify(request)));
    ...
    const replay = await projectionService.create(JSON.parse(JSON.stringify(request)));
    ...
    const workflowResult = await workflowService.createOrderProjection(JSON.parse(JSON.stringify(request)));
    ...
    await assert.rejects(
        ...
        (error) => error.code === 'ERR_ORD_00024' && error.message.includes('cart enterprise does not match request')
    );
