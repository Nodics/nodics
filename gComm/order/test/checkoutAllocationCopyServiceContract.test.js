...
const serviceStore = function (store) {
  return {
    get: async (request) => ({
      result: store.filter(
        (item) => item.orderCode === request.query.orderCode,
      ),
    }),
    save: async (request) => {
...
const request = {
  ...
  cartDeliveryGroups: [
    ...
    },
    {
      entCode: "enterpriseA",
      cartCode: "cart-1",
      deliveryGroupCode: "pickup-store",
      groupType: "PICKUP",
      addressCode: "store-1",
      status: "ACTIVE",
    },
  ...
  cartDeliveryAllocations: [
    ...
    },
    {
      entCode: "enterpriseA",
      cartCode: "cart-1",
      allocationCode: "delivery-a2",
      entryCode: "entry-1",
      deliveryGroupCode: "pickup-store",
      quantity: "1",
      unitCode: "EA",
      serialNumbers: ["serial-3"],
      status: "ACTIVE",
    },
  ],
...
(async () => {
  assert.strictEqual(
    orderProperties.order.checkoutPlacement.allocationCopy.enabled,
    true,
  );
  assert.strictEqual(
    orderProperties.order.checkoutPlacement.allocationCopy.targetStatus,
    "ORDERED",
  );
  ...
  assert.strictEqual(
    savedDeliveryGroups[0].sourceDeliveryGroupCode,
    "ship-home",
  );
  assert.strictEqual(
    savedDeliveryGroups[0].deliveryChargeQuoteCode,
    "delivery-quote-1",
  );
  assert.strictEqual(savedDeliveryGroups[0].deliveryChargeAmount, "5.00");
  assert.strictEqual(savedDeliveryGroups[0].deliveryChargeCurrencyCode, "USD");
  assert.strictEqual(savedDeliveryGroups[0].status, "ORDERED");
  assert.strictEqual(
    savedPaymentGroups[1].sourcePaymentGroupCode,
    "cod-balance",
  );
  assert.strictEqual(
    savedDeliveryAllocations[1].sourceAllocationCode,
    "delivery-a2",
  );
  assert.strictEqual(
    savedDeliveryAllocations[1].sourceDeliveryGroupCode,
    "pickup-store",
  );
  assert.deepStrictEqual(savedDeliveryAllocations[1].serialNumbers, [
    "serial-3",
  ]);
  assert.strictEqual(
    savedPaymentAllocations[0].sourcePaymentGroupCode,
    "card-main",
  );
  ...
  assert.strictEqual(
    workflowResult.feedback.allocationCopy.orderCode,
    "order::checkout-1",
  );
