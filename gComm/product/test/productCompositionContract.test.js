...
(async () => {
    ...
    assert(!schemas.productMediaReference.definition.uri, 'Product must not store media storage or delivery URI');
    assert(schemas.productMediaReference.definition.mediaReferenceCode.required, 'Product media assignment must have its own Product-owned identity');
    assert(schemas.productMediaReference.definition.mediaSetCode, 'Product media assignment must support nMedia mediaSet references');
    ...
    let mediaRequest = { tenant: 't1', authData, model: { catalogCode: 'retail', mediaReferenceCode: 'phone-primary', mediaSetCode: 'phone-primary-set', itemType: 'PRODUCT', itemCode: 'phone', mediaType: 'IMAGE', role: 'PRIMARY', localeCode: 'en-AE', position: 10, altText: 'Phone', status: 'ACTIVE' } }; await validator.prepareMedia(mediaRequest);
    assert.strictEqual(mediaRequest.model.code, 'entA::mediaReference::retail::phone-primary');
    ...
    await assert.rejects(validator.prepareMedia({ tenant: 't1', authData, model: Object.assign({}, mediaRequest.model, { code: undefined, mediaReferenceCode: 'unsafe-both', mediaCode: 'phone-primary-file' }) }), error => error.code === 'ERR_PRODUCT_00037');
    await assert.rejects(validator.prepareMedia({ tenant: 't1', authData, model: Object.assign({}, mediaRequest.model, { code: undefined, mediaReferenceCode: 'unsafe-empty', mediaSetCode: undefined }) }), error => error.code === 'ERR_PRODUCT_00037');
