...
module.exports = {
    ...
    /** Builds bounded Product search documents without price, stock, or media-storage truth. */ documents: function (manifest) { let grouped = this.grouped(manifest), maximum = Number(this.policy().maximumDocuments || 20000), documents = (grouped.productItem || []).filter(item => item.status === 'ACTIVE').map(item => { let matches = model => model.itemType === item.itemType && model.itemCode === item.itemCode; return { code: item.code, enterpriseCode: item.enterpriseCode, catalogCode: item.catalogCode, itemType: item.itemType, itemCode: item.itemCode, name: item.name, description: item.description, sellable: item.sellable, stockManaged: item.stockManaged, categoryCodes: (grouped.productCategoryAssignment || []).filter(matches).map(model => model.categoryCode), attributes: (grouped.productAttributeValue || []).filter(matches).map(model => ({ attributeCode: model.attributeCode, localeCode: model.localeCode, stringValue: model.stringValue, booleanValue: model.booleanValue, integerValue: model.integerValue, decimalValue: model.decimalValue, referenceCode: model.referenceCode, unitCode: model.unitCode })), media: (grouped.productMediaReference || []).filter(matches).map(model => ({ mediaReferenceCode: model.mediaReferenceCode, mediaCode: model.mediaCode, mediaSetCode: model.mediaSetCode, mediaType: model.mediaType, role: model.role, localeCode: model.localeCode, position: model.position, altText: model.altText })), releaseVersion: manifest.code }; }); if (documents.length > maximum) throw new CLASSES.NodicsError('ERR_PRODUCT_00066', 'Product search projection exceeds configured bounds'); return documents; },
