...
module.exports = {
  ...
  error: function (message) {
    if (typeof CLASSES !== "undefined" && CLASSES.NodicsError)
      return new CLASSES.NodicsError(message, null, "ERR_PAY_00004");
  ...
  existingTransactionByQuery: async function (request, query) {
    ...
    if (transactions.length > 1)
      throw this.error("Payment refund resolved duplicate idempotency records");
    return transactions[0];
  },
  /** Loads an existing refund transaction by idempotency key. */
  existingTransaction: async function (request, idempotencyKey) {
    return this.existingTransactionByQuery(request, {
  ...
  assertRecoveryRequest: function (request) {
    if (
      ...
      throw this.error(
        "Payment refund recovery requires tenant, auth, orderCode, and entCode",
      );
    }
    if (
      ...
      throw this.error(
        "Payment refund recovery request must not contain raw credentials, raw provider payloads, or card data",
      );
    }
    if (
      !SERVICE.DefaultPaymentPolicyService ||
      typeof SERVICE.DefaultPaymentPolicyService.buildRefundDraft !== "function"
    ) {
  ...
  refund: async function (request) {
    if (
      ...
      throw this.error(
        "Payment refund requires tenant, auth, orderCode, and entCode",
      );
    }
    if (
      !SERVICE.DefaultPaymentPolicyService ||
    ...
    if (
      ...
      typeof SERVICE.DefaultPaymentProviderGatewayService.refund !== "function"
    ) {
      throw this.error(
        "Payment provider refund gateway service is unavailable",
      );
    }
  ...
  retryRefund: async function (request) {
    ...
    if (recovery.enabled === false)
      throw this.error("Payment refund recovery is disabled");
    let draft = SERVICE.DefaultPaymentPolicyService.buildRefundDraft(request);
    let existing = await this.existingTransaction(
      request,
      draft.idempotencyKey,
      ...
      return Object.assign(
        {
          idempotent: true,
          recovered: true,
          recoveryAction: "NO_RETRY_REQUIRED",
        },
        existing,
      );
    }
    ...
    if (existing && !retryStatuses.includes(existing.status)) {
      throw this.error(
        "Payment refund recovery cannot retry transaction in status " +
          existing.status,
      );
    }
    ...
    return Object.assign(
      { recovered: true, recoveryAction: "RETRY_REFUND" },
      saved,
    );
  },
  ...
  reconcileRefund: async function (request) {
    ...
    let draft = SERVICE.DefaultPaymentPolicyService.buildRefundDraft(request);
    let existing =
      (await this.existingTransactionByCode(
        request,
        request.refundTransactionCode || request.transactionCode,
      )) || (await this.existingTransaction(request, draft.idempotencyKey));
    if (!existing)
      throw this.error(
        "Payment refund recovery could not find transaction evidence to reconcile",
      );
    let recovery = this.config().refundRecovery || {};
    let terminalStatuses = recovery.terminalSuccessStatuses || ["REFUNDED"];
    let result = {
