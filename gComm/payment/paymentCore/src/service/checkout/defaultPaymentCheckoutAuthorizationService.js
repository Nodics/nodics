...
module.exports = {
  ...
  error: function (message) {
    if (typeof CLASSES !== "undefined" && CLASSES.NodicsError)
      return new CLASSES.NodicsError(message, null, "ERR_PAY_00003");
  ...
  loadPaymentGroups: async function (request) {
    ...
    if (request.allocationCopy && request.allocationCopy.paymentGroups)
      return this.items(request.allocationCopy.paymentGroups);
    if (
      request.feedback &&
      request.feedback.allocationCopy &&
      request.feedback.allocationCopy.paymentGroups
    ) {
      return this.items(request.feedback.allocationCopy.paymentGroups);
    }
    if (
      !SERVICE.DefaultOrderPaymentGroupService ||
      typeof SERVICE.DefaultOrderPaymentGroupService.get !== "function"
    )
      return [];
  ...
  existingTransaction: async function (request, idempotencyKey) {
    if (
      ...
      typeof SERVICE.DefaultPaymentTransactionService.get !== "function"
    )
      return undefined;
    ...
    if (transactions.length > 1)
      throw this.error(
        "Payment authorization resolved duplicate idempotency records",
      );
  ...
  saveTransaction: async function (request, transaction) {
    if (
      !SERVICE.DefaultPaymentTransactionService ||
      typeof SERVICE.DefaultPaymentTransactionService.save !== "function"
    ) {
  ...
  authorizeGroup: async function (request, paymentGroup) {
    if (
      ...
      typeof SERVICE.DefaultPaymentPolicyService.buildAuthorizationDraft !==
        "function"
    ) {
    ...
    if (
      ...
      typeof SERVICE.DefaultPaymentProviderGatewayService.authorize !==
        "function"
    ) {
      throw this.error("Payment provider gateway service is unavailable");
    }
    let draft = SERVICE.DefaultPaymentPolicyService.buildAuthorizationDraft(
      request,
      paymentGroup,
    );
    let existing = await this.existingTransaction(
      request,
      draft.idempotencyKey,
    );
    if (existing) return Object.assign({ idempotent: true }, existing);
    let providerResult =
      await SERVICE.DefaultPaymentProviderGatewayService.authorize(
  ...
  authorize: async function (request) {
    if (
      ...
      throw this.error(
        "Payment authorization requires tenant, auth, orderCode, and entCode",
      );
    }
    let paymentGroups = await this.loadPaymentGroups(request);
    if (!paymentGroups.length)
      throw this.error("Payment authorization requires order payment groups");
    let authorized = [];
    let deferred = [];
    let failed = [];
    for (let paymentGroup of paymentGroups) {
      try {
        ...
        failed.push({
          ...
          failureMessage: String(error.message || error).slice(
            0,
            Number(this.config().failureMessageLimit || 240),
          ),
        });
      }
    }
    if (failed.length)
      throw this.error(
        "Payment authorization failed for one or more payment groups",
      );
    return {
