...
module.exports = {
  ...
  error: function (message) {
    if (typeof CLASSES !== "undefined" && CLASSES.NodicsError)
      return new CLASSES.NodicsError(message, null, "ERR_PAY_00001");
  ...
  validateMoney: function (value) {
    ...
    let expression = new RegExp(
      policy.moneyPattern || "^(0|[1-9][0-9]*)(\\.[0-9]+)?$",
    );
    ...
    return (
      total <= Number(policy.maximumDigits || 38) &&
  ...
  providerCode: function (paymentModeCode) {
    if (
      ...
      typeof SERVICE.DefaultPaymentProviderRegistryService
        .defaultProviderCode === "function"
    ) {
      return SERVICE.DefaultPaymentProviderRegistryService.defaultProviderCode(
        paymentModeCode,
      );
    }
  ...
  operation: function (paymentModeCode) {
    if (
      ...
      return SERVICE.DefaultPaymentMethodPolicyService.operation(
        paymentModeCode,
      );
    }
    let policy = this.policy();
    if ((policy.deferredPaymentModes || []).includes(paymentModeCode))
      return "DEFER";
    return "AUTHORIZE";
  },
  ...
  prepareProvider: function (request) {
    ...
    ["enterpriseCode", "providerCode", "providerType", "displayName"].forEach(
      (field) => {
        if (!model[field])
          throw this.error("Payment Provider " + field + " is required");
      },
    );
    if (!Array.isArray(model.methodCodes) || !model.methodCodes.length) {
      if (Array.isArray(model.paymentModes) && model.paymentModes.length)
        model.methodCodes = model.paymentModes.slice();
    ...
    if (!Array.isArray(model.operations) || !model.operations.length)
      throw this.error("Payment Provider operations are required");
    model.operations.forEach((operation) => {
      if (!(this.policy().operations || []).includes(operation))
        throw this.error("Payment Provider operation is unsupported");
    });
    model.adapterService =
      model.adapterService || "DefaultManualPaymentProviderAdapterService";
    model.policyService =
      model.policyService || "DefaultPaymentProviderPolicyService";
    if (!/^[A-Za-z][A-Za-z0-9]*Service$/.test(model.adapterService))
      throw this.error("Payment Provider adapterService is invalid");
    if (!/^[A-Za-z][A-Za-z0-9]*Service$/.test(model.policyService))
      throw this.error("Payment Provider policyService is invalid");
    if (
      JSON.stringify(model).match(
        /cvv|cardNumber|pan|secret|password|rawGateway|gatewayPayload|providerPayload|apiKey|accessToken|refreshToken/i,
      )
    ) {
      throw this.error(
        "Payment Provider must not store raw credentials, raw provider payloads, or card data",
      );
    }
  ...
  prepareMethod: function (request) {
    let model = Object.assign({}, (request || {}).model || {});
    ["enterpriseCode", "methodCode", "displayName", "defaultOperation"].forEach(
      (field) => {
        if (!model[field])
          throw this.error("Payment Method " + field + " is required");
      },
    );
    if (!(this.policy().operations || []).includes(model.defaultOperation))
      throw this.error("Payment Method defaultOperation is unsupported");
    if (
      JSON.stringify(model).match(
      ...
      throw this.error(
        "Payment Method must not store raw credentials, raw provider payloads, or card data",
      );
    }
  ...
  prepareTransaction: function (request) {
    ...
    [
      ...
      if (!model[field])
        throw this.error("Payment Transaction " + field + " is required");
    });
    if (!(this.policy().operations || []).includes(model.operation))
      throw this.error("Payment operation is unsupported");
    if (
      !(this.policy().transactionStatuses || []).includes(
        model.status || "REQUESTED",
      )
    )
      throw this.error("Payment transaction status is unsupported");
    if (!this.validateMoney(model.amount))
      throw this.error(
        "Payment amount must be an exact non-negative decimal string",
      );
    if (
      JSON.stringify(model).match(
        /cvv|cardNumber|pan|secret|password|rawGateway|gatewayPayload|providerPayload/i,
      ...
      throw this.error(
        "Payment Transaction must not store raw credentials, raw provider payloads, or card data",
      );
    }
  ...
  buildAuthorizationDraft: function (request, paymentGroup) {
    ...
    let idempotencyKey = [
      request.idempotencyKey ||
        (request.workflowCarrier && request.workflowCarrier.code) ||
        request.orderCode,
    ...
    return this.prepareTransaction({
      model: {
        enterpriseCode:
          ...
          paymentGroup.entCode ||
          paymentGroup.enterpriseCode,
        ...
        operation: operation,
        amount:
          paymentGroup.plannedAmount ||
          paymentGroup.authorizedAmount ||
          paymentGroup.capturedAmount ||
          "0",
  ...
  buildRefundDraft: function (request) {
    if (
      ...
      throw this.error(
        "Payment refund request must not contain raw credentials, raw provider payloads, or card data",
      );
    }
    let idempotencyKey = [
    ...
    return this.prepareTransaction({
      model: {
        ...
        transactionCode:
          request.transactionCode || "payment::" + idempotencyKey,
        idempotencyKey: idempotencyKey,
        providerCode:
          request.providerCode || this.providerCode(request.paymentModeCode),
        ...
        paymentEvidenceCode:
          request.paymentEvidenceCode || "refund-evidence::" + idempotencyKey,
        status: "REQUESTED",
      },
    });
  },
  ...
  rejectHardDelete: function () {
    return Promise.reject(
      this.error(
        "Payment transaction evidence cannot be hard-deleted; use lifecycle states",
      ),
    );
  },
};
