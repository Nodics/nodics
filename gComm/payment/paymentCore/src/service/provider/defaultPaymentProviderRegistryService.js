...
module.exports = {
  ...
  error: function (message) {
    if (typeof CLASSES !== "undefined" && CLASSES.NodicsError)
      return new CLASSES.NodicsError(message, null, "ERR_PAY_00007");
  ...
  isSelectableStatus: function (provider) {
    return (
      provider &&
      !["DRAFT", "SUSPENDED", "INACTIVE", "RETIRED"].includes(provider.status)
    );
  },
  ...
  normalizeRecord: function (record, configured) {
    ...
    if (!provider.providerCode && configured && configured.providerCode)
      provider.providerCode = configured.providerCode;
    if (
      Array.isArray(provider.paymentModes) &&
      !Array.isArray(provider.methodCodes)
    )
      provider.methodCodes = provider.paymentModes.slice();
    if (
      Array.isArray(provider.methodCodes) &&
      !Array.isArray(provider.paymentModes)
    )
      provider.paymentModes = provider.methodCodes.slice();
    provider.adapterService =
      provider.adapterService ||
      (configured && configured.adapterService) ||
      "DefaultManualPaymentProviderAdapterService";
    provider.policyService =
      provider.policyService ||
      (configured && configured.policyService) ||
      "DefaultPaymentProviderPolicyService";
    provider.status = provider.status || "ACTIVE";
    provider.configurationSource =
      record && record.providerCode
        ? "GOVERNED_RECORD"
  ...
  recordProvider: async function (providerCode, request) {
    ...
    let items = this.items(response).filter((item) =>
      this.isGovernedOverrideStatus(item),
    );
    if (items.length > 1)
      throw this.error(
        "Multiple active Payment Provider records found for " + providerCode,
      );
    return items[0];
  },
  ...
  recordProviders: async function (request) {
    ...
    let response = await SERVICE.DefaultPaymentProviderService.get({
      ...
      searchOptions: {
        limit: Number(this.policy().axisProviderRecordLimit || 500),
      },
    });
  ...
  provider: function (providerCode) {
    ...
    if (configured)
      return this.normalizeRecord({ providerCode: code }, configured);
    return {
      providerCode: code,
  ...
  defaultProviderCode: function (paymentModeCode) {
    let method =
      ...
      typeof SERVICE.DefaultPaymentMethodPolicyService.method === "function"
  ...
  eligibleProviders: function (paymentModeCode) {
    ...
    return Object.keys(providers)
      ...
      .filter((provider) =>
        (provider.methodCodes || provider.paymentModes || []).includes(
          paymentModeCode,
        ),
      );
  },
  ...
  assertSupports: function (provider, paymentModeCode, operation) {
    if (!this.isSelectableStatus(provider))
      throw this.error("Payment provider is not active for payment execution");
    let methodCodes = provider.methodCodes || provider.paymentModes || [];
    if (methodCodes.length && !methodCodes.includes(paymentModeCode)) {
      throw this.error(
        "Payment provider does not support payment method " + paymentModeCode,
      );
    }
    if (
      ...
      throw this.error(
        "Payment provider does not support operation " + operation,
      );
    }
    return provider;
  },
