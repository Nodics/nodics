...
module.exports = {
  ...
  error: function (message, code) {
    if (typeof CLASSES !== "undefined" && CLASSES.NodicsError)
      return new CLASSES.NodicsError(message, null, code || "ERR_PAY_00009");
  ...
  provider: async function (request) {
    ...
    if (model && model.providerCode) {
      return SERVICE.DefaultPaymentPolicyService.prepareProvider({
        model: Object.assign({}, model),
      });
    }
    let providerCode = body.providerCode || (request || {}).providerCode;
    if (!providerCode)
      throw this.error("Payment Provider lifecycle requires providerCode");
    ...
    if (registry && typeof registry.provider === "function")
      return registry.provider(providerCode);
  ...
  execute: async function (request) {
    ...
    if (!actionId)
      throw this.error("Payment Provider lifecycle actionId is required");
  ...
  validateProvider: async function (request) {
    ...
    SERVICE.DefaultPaymentPolicyService.prepareProvider({
      model: Object.assign({}, provider),
    });
    let adapterAvailable =
      typeof SERVICE !== "undefined" &&
      provider.adapterService &&
      SERVICE[provider.adapterService];
    let registered =
      typeof SERVICE !== "undefined" &&
    ...
    if (!adapterAvailable && !registered)
      throw this.error(
        "Payment Provider adapter is not active for " + provider.providerCode,
      );
    let connectorValidation =
      SERVICE.DefaultPaymentProviderConnectorPolicyService
        ? SERVICE.DefaultPaymentProviderConnectorPolicyService.validateProvider(
            provider,
          )
        : {
            connectorReferencePresent: !!(
              provider.connectorCode || provider.configRef
            ),
          };
  ...
  testProvider: async function (request) {
    ...
    let operation = (provider.operations || []).includes("RECONCILE")
      ? "RECONCILE"
      : (provider.operations || [])[0] || "AUTHORIZE";
    let mode = (provider.methodCodes ||
      provider.paymentModes || ["OFFLINE"])[0];
    let evidence =
      await SERVICE.DefaultPaymentProviderGatewayService.executeOperation(
        {
          enterpriseCode:
            provider.enterpriseCode ||
        ...
        String(operation).toLowerCase() === "reconcile"
          ? "reconcile"
          : "authorize",
        [operation],
      );
  ...
  activateProvider: async function (request) {
    ...
    await this.validateProvider(
      Object.assign({}, request, {
        model: Object.assign({}, provider, { status: "ACTIVE" }),
      }),
    );
  ...
  requestConnectorRotation: async function (request) {
    ...
    let rotation = SERVICE.DefaultPaymentProviderConnectorPolicyService
      ? SERVICE.DefaultPaymentProviderConnectorPolicyService.rotationRequest(
          provider,
        )
      : {
