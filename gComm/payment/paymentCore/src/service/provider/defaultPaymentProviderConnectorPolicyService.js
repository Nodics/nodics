...
module.exports = {
    ...
    policy: function () { return (((CONFIG.get('payment') || {}).paymentPolicy || {}).connectorPolicy) || {}; },
    /** Creates a stable connector-policy error. */
    error: function (message) {
        if (typeof CLASSES !== 'undefined' && CLASSES.NodicsError) return new CLASSES.NodicsError(message, null, 'ERR_PAY_00010');
    ...
    requiresReference: function (provider) {
        if (!((this.policy() || {}).requireConnectorReferenceForGateway !== false)) return false;
        return ['CARD_GATEWAY', 'WALLET', 'CARD_NETWORK', 'PROJECT_PROVIDER'].includes(provider && provider.providerType);
    },
    /** Validates one safe string reference. */
    validateReference: function (value, pattern, label) {
        if (value === undefined || value === null || value === '') return true;
        if (typeof value !== 'string') throw this.error(label + ' must be a safe string reference');
        let forbidden = (this.policy().forbiddenReferenceTerms || []).map((term) => String(term).toLowerCase());
        let normalized = value.toLowerCase();
        if (forbidden.some((term) => normalized.includes(term))) throw this.error(label + ' must not contain credential-like terms');
        if (!(new RegExp(pattern)).test(value)) throw this.error(label + ' format is invalid');
        return true;
    },
    ...
    validateProvider: function (provider) {
        ...
        this.validateReference(connectorCode, policy.connectorCodePattern || '^[A-Za-z][A-Za-z0-9._:-]{0,127}$', 'Payment Provider connectorCode');
        this.validateReference(configRef, policy.configRefPattern || '^[A-Za-z][A-Za-z0-9._:-]{0,255}$', 'Payment Provider configRef');
        if (this.requiresReference(provider) && !connectorCode && !configRef) {
            throw this.error('Payment Provider connectorCode or configRef is required for gateway providers');
        }
        return {
            connectorReferencePresent: !!(connectorCode || configRef),
            rotationAuthority: policy.rotationAuthority || 'connector-secret-authority',
