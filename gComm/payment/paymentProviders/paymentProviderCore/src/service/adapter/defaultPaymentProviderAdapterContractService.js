...
module.exports = {
  ...
  validate: function (providerCode, adapter) {
    if (!providerCode || !adapter || typeof adapter !== "object") {
      throw new Error(
        "Payment provider adapter requires providerCode and adapter object",
      );
    }
    if (adapter.providerCode && adapter.providerCode !== providerCode) {
      throw new Error(
        "Payment provider adapter code mismatch: " + providerCode,
      );
    }
  ...
  normalizeResult: function (result) {
    if (!result || typeof result !== "object")
      throw new Error("Payment provider result is required");
    [
      "transactionCode",
      "idempotencyKey",
      "providerCode",
      ...
      if (!result[field])
        throw new Error("Payment provider result missing " + field);
    });
    if (!ALLOWED_STATUSES.includes(result.status)) {
      throw new Error(
        "Payment provider returned unsupported status " + result.status,
      );
    }
    ["rawGatewayPayload", "cardNumber", "cvv", "secret", "credential"].forEach(
      (unsafeField) => {
        if (result[unsafeField] !== undefined) {
          throw new Error(
            "Payment provider result exposed unsafe field " + unsafeField,
          );
        }
      },
    );
