...
const DEFAULT_POLICY = {
    ...
    maximumDigits: 38,
    maximumScale: 18,
    deliveryGroupStatuses: ['DRAFT', 'ACTIVE', 'ALLOCATED', 'RELEASED', 'CANCELLED'],
    paymentGroupStatuses: ['DRAFT', 'ACTIVE', 'AUTHORIZED', 'CAPTURED', 'PARTIALLY_REFUNDED', 'REFUNDED', 'CANCELLED'],
    allocationStatuses: ['ACTIVE', 'RESERVED', 'ALLOCATED', 'RELEASED', 'CANCELLED'],
    deliveryGroupRequiredFields: ['entCode', 'deliveryGroupCode'],
    paymentGroupRequiredFields: ['entCode', 'paymentGroupCode', 'paymentModeCode', 'currencyCode'],
    allocationRequiredFields: ['entCode', 'allocationCode', 'entryCode', 'quantity', 'unitCode'],
    deliveryGroupTypes: ['ADDRESS', 'PICKUP', 'DIGITAL', 'SERVICE'],
    amountFields: ['amount', 'plannedAmount', 'authorizedAmount', 'capturedAmount', 'refundedAmount'],
    conversion: {
...
const validateDeliveryGroup = function (group, policy, options) {
    ...
    const activeOptions = Object.assign({ parentField: undefined }, options || {});
    const model = Object.assign({}, group || {});
    const errors = [];

    requireFields(model, activePolicy.deliveryGroupRequiredFields, errors);
    if (activeOptions.parentField && !model[activeOptions.parentField]) errors.push(activeOptions.parentField + ' is required');
    if (!(activePolicy.deliveryGroupTypes || []).includes(model.groupType || 'ADDRESS')) errors.push('groupType is not allowed');
    if (!(activePolicy.deliveryGroupStatuses || []).includes(model.status || 'DRAFT')) errors.push('status is not allowed');
...
const validatePaymentGroup = function (group, policy, options) {
    ...
    if (activeOptions.parentField && !model[activeOptions.parentField]) errors.push(activeOptions.parentField + ' is required');
    if (!(activePolicy.paymentGroupStatuses || []).includes(model.status || 'DRAFT')) errors.push('status is not allowed');

    ['plannedAmount', 'authorizedAmount', 'capturedAmount', 'refundedAmount'].forEach((field) => {
...
const validateAllocation = function (allocation, policy, options) {
    const activePolicy = mergePolicy(policy);
    const activeOptions = Object.assign({ parentField: undefined, groupField: undefined, amountRequired: false }, options || {});
    const model = Object.assign({}, allocation || {});
    const errors = [];

    requireFields(model, activePolicy.allocationRequiredFields, errors);
    if (activeOptions.parentField && !model[activeOptions.parentField]) errors.push(activeOptions.parentField + ' is required');
    if (activeOptions.groupField && !model[activeOptions.groupField]) errors.push(activeOptions.groupField + ' is required');
    ...
    if (activeOptions.amountRequired && !model.amount) errors.push('amount is required');
    if (model.amount !== undefined && !validateDecimal(model.amount, activePolicy.moneyPattern, activePolicy, { positive: false })) {
        errors.push('amount must be an exact non-negative decimal string');
    }
    if (model.serialNumbers !== undefined && !Array.isArray(model.serialNumbers)) {
        errors.push('serialNumbers must be an array');
    }
    if (!(activePolicy.allocationStatuses || []).includes(model.status || 'ACTIVE')) errors.push('status is not allowed');

    return {
        valid: errors.length === 0,
        errors: errors,
        model: Object.assign({}, model, {
            serialNumbers: Array.isArray(model.serialNumbers) ? model.serialNumbers : [],
...
const addScaledDecimal = function (left, right, scale) {
    return toScaledInteger(left || '0', scale) + toScaledInteger(right || '0', scale);
};

const validateAllocationTotals = function (entries, allocations, policy) {
    const activePolicy = mergePolicy(policy);
    const scale = Number(activePolicy.maximumScale || DEFAULT_POLICY.maximumScale);
    ...
    (allocations || []).forEach((allocation) => {
        ...
        if (!entry) {
            errors.push('allocation ' + allocation.allocationCode + ' references a missing entry');
            return;
        }
        if (!validateDecimal(allocation.quantity, activePolicy.quantityPattern, activePolicy, { positive: true })) {
            errors.push('allocation ' + allocation.allocationCode + ' quantity is invalid');
            return;
        }
        const current = totalsByEntry.get(allocation.entryCode) || BigInt(0);
        totalsByEntry.set(allocation.entryCode, current + toScaledInteger(allocation.quantity, scale));
    });

    totalsByEntry.forEach((allocated, entryCode) => {
        const entry = entryByCode.get(entryCode);
        if (!validateDecimal(entry.quantity, activePolicy.quantityPattern, activePolicy, { positive: true })) {
            errors.push('entry ' + entryCode + ' quantity is invalid');
            return;
        }
        if (allocated > toScaledInteger(entry.quantity, scale)) {
            errors.push('entry ' + entryCode + ' allocation quantity exceeds entry quantity');
        }
    });
...
const buildOrderModelFromCartModel = function (cartModel, context, options) {
    ...
    target[activeOptions.targetParentField] = activeContext[activeOptions.targetParentField];
    target[activeOptions.sourceParentField] = source[activeOptions.sourceParentField];
    if (activeOptions.targetStatus) target.status = activeContext.status || activeOptions.targetStatus;
    return target;
};
...
const buildOrderDeliveryGroupFromCartDeliveryGroup = function (cartDeliveryGroup, orderContext, policy) {
    const activePolicy = mergePolicy(policy);
    ...
    target.sourceDeliveryGroupCode = cartDeliveryGroup && cartDeliveryGroup.deliveryGroupCode;
    target.deliveryGroupCode = resolveMappedCode(target.deliveryGroupCode, (orderContext || {}).deliveryGroupCodeMap);
    return target;
};

const buildOrderPaymentGroupFromCartPaymentGroup = function (cartPaymentGroup, orderContext, policy) {
    const activePolicy = mergePolicy(policy);
    const conversion = activePolicy.conversion || DEFAULT_POLICY.conversion;
    ...
    target.sourcePaymentGroupCode = cartPaymentGroup && cartPaymentGroup.paymentGroupCode;
    target.paymentGroupCode = resolveMappedCode(target.paymentGroupCode, (orderContext || {}).paymentGroupCodeMap);
    return target;
};

const buildOrderDeliveryAllocationFromCartDeliveryAllocation = function (cartDeliveryAllocation, orderContext, policy) {
    const activePolicy = mergePolicy(policy);
    const conversion = activePolicy.conversion || DEFAULT_POLICY.conversion;
    const target = buildOrderModelFromCartModel(cartDeliveryAllocation, orderContext, {
        sourceParentField: conversion.sourceParentField,
        targetParentField: conversion.targetParentField,
        targetStatus: conversion.targetStatus,
    ...
    target.sourceAllocationCode = cartDeliveryAllocation && cartDeliveryAllocation.allocationCode;
    target.sourceDeliveryGroupCode = cartDeliveryAllocation && cartDeliveryAllocation.deliveryGroupCode;
    target.allocationCode = resolveMappedCode(target.allocationCode, (orderContext || {}).allocationCodeMap);
    target.deliveryGroupCode = resolveMappedCode(target.deliveryGroupCode, (orderContext || {}).deliveryGroupCodeMap);
    return target;
};

const buildOrderPaymentAllocationFromCartPaymentAllocation = function (cartPaymentAllocation, orderContext, policy) {
    const activePolicy = mergePolicy(policy);
    const conversion = activePolicy.conversion || DEFAULT_POLICY.conversion;
    const target = buildOrderModelFromCartModel(cartPaymentAllocation, orderContext, {
        sourceParentField: conversion.sourceParentField,
        targetParentField: conversion.targetParentField,
        targetStatus: conversion.targetStatus,
    ...
    target.sourceAllocationCode = cartPaymentAllocation && cartPaymentAllocation.allocationCode;
    target.sourcePaymentGroupCode = cartPaymentAllocation && cartPaymentAllocation.paymentGroupCode;
    target.allocationCode = resolveMappedCode(target.allocationCode, (orderContext || {}).allocationCodeMap);
    target.paymentGroupCode = resolveMappedCode(target.paymentGroupCode, (orderContext || {}).paymentGroupCodeMap);
    return target;
};
