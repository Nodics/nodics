...
const validateEntry = function (entry, policy, options) {
  ...
  const activeOptions = Object.assign(
    { parentField: undefined, requireParent: true },
    options || {},
  );
  const model = entry || {};
  const errors = [];
  ...
  if (
    activeOptions.requireParent &&
    activeOptions.parentField &&
    !model[activeOptions.parentField]
  ) {
    errors.push(activeOptions.parentField + " is required");
  }

  if (
    !Number.isInteger(Number(model.lineNumber)) ||
    Number(model.lineNumber) <= 0
  ) {
  ...
  if (
    ...
    !(
      activePolicy.taxInclusionModes || DEFAULT_POLICY.taxInclusionModes
    ).includes(model.taxInclusionMode)
  ) {
    errors.push("taxInclusionMode is not allowed");
  }

  if (
    model.taxIncluded !== undefined &&
    typeof model.taxIncluded !== "boolean"
  ) {
    errors.push("taxIncluded must be boolean");
  }

  const normalized = Object.assign({}, model, {
    status: model.status || "ACTIVE",
  });
  if (normalized.taxInclusionMode) {
    normalized.taxIncluded = normalized.taxInclusionMode === "TAX_INCLUSIVE";
  }
...
const validateUpdate = function (current, patch, policy) {
  ...
  (activePolicy.immutableFields || []).forEach((field) => {
    if (
      ...
      JSON.stringify(update[field]) !== JSON.stringify(currentModel[field])
    ) {
  ...
  if (update.status && update.status !== currentModel.status) {
    const allowed =
      (activePolicy.allowedTransitions || {})[currentModel.status] || [];
  ...
  return validateEntry(merged, activePolicy, {
    parentField:
      currentModel.orderCode !== undefined ? "orderCode" : "cartCode",
    requireParent: false,
  }).errors.concat(errors);
};

const buildOrderEntryFromCartEntry = function (
  cartEntry,
  orderContext,
  policy,
) {
  ...
  target[conversion.sourceParentField || "cartCode"] =
    source[conversion.sourceParentField || "cartCode"];
  target.status = context.status || conversion.targetStatus || "ORDERED";

  if (context.entryCodePrefix)
    target.entryCode = context.entryCodePrefix + target.entryCode;
  if (context.allocationCode) target.allocationCode = context.allocationCode;
  if (context.reservationCode || source.reservationCode)
    target.reservationCode = context.reservationCode || source.reservationCode;
