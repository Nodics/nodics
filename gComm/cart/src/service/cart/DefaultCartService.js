 ...
 * @description Cart service implementation that prepares cart persistence context, starts create/calculation pipelines, and wraps cart read errors.
 * @layer service
 * @owner cart
...
module.exports = {
  ...
  createCart: function (request) {
    ...
    return new Promise((resolve, reject) => {
      SERVICE.DefaultPipelineService.start("createCartPipeline", request, {})
        .then((success) => {
          resolve(success);
        })
        .catch((error) => {
  ...
  /**
   * Calculates a cart through the configured aggregate cart calculation pipeline.
   *
   * @param {Object} request Nodics request context.
   * @param {string} request.tenant Active tenant.
   * @param {Object} [request.model] Optional cart aggregate or calculation payload.
   * @param {string} [request.cartCode] Cart code to calculate.
   * @returns {Promise<Object>} Calculation pipeline result.
   * @sideEffects Sets `request.calculationPipelineName` and normalized `request.cartCode`.
   * @throws Wraps pipeline errors in `ERR_ORD_00000`.
   */
  calculateCart: function (request) {
    let model = request.model || request.body || {};
    request.cartCode =
      request.cartCode || model.cartCode || model.code || request.code;
    request.entCode =
      request.entCode ||
      model.entCode ||
      (request.authData && request.authData.entCode);
    request.calculationPipelineName =
      (((CONFIG.get("cart") || {}).calculation || {}).cartPipeline || {})
