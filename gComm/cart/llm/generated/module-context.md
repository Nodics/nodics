...
| `gComm/cart/src/controller/defaultCartController.js` | `src` | `documented` | 7/7 | HTTP-facing cart controller that maps secured cart routes to cart facade operations. |  |
| `gComm/cart/src/facade/DefaultCartFacade.js` | `src` | `documented` | 5/5 | Facade boundary for cart operations, keeping controllers independent from cart service implementation details. |  |
...
| `gComm/cart/src/service/cart/DefaultCartService.js` | `src` | `documented` | 5/5 | Cart service implementation that prepares cart persistence context, starts create/calculation pipelines, and wraps cart read errors. |  |
