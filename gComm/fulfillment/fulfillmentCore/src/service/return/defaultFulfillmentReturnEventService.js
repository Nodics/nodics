...
module.exports={init:function(){return Promise.resolve(true);},postInit:function(){return Promise.resolve(true);},publish:async function(request,model,eventType){if(!SERVICE.DefaultEventService||typeof SERVICE.DefaultEventService.handleEvent!=='function')return{published:false};let event={tenant:request.tenant,active:true,event:'fulfillmentReturn.'+String(eventType).toLowerCase(),sourceName:'fulfillment',sourceId:model.returnCode,target:'fulfillmentReturn',state:'NEW',type:'ASYNC',data:{eventCode:[model.returnCode,eventType].join('::'),returnCode:model.returnCode,orderCode:model.orderCode,enterpriseCode:model.entCode||model.enterpriseCode,consignmentCode:model.consignmentCode,shipmentCode:model.shipmentCode,status:model.status,dispositionCode:model.dispositionCode,correlationId:model.orderLifecycleRequestCode||model.returnCode,occurredAt:new Date()}};await SERVICE.DefaultEventService.handleEvent({tenant:request.tenant,event:event});return{published:true,eventCode:event.data.eventCode};}};
