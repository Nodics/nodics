...
/**
 * @module fulfillment/controller/operations/DefaultFulfillmentOperationsLifecycleController
 * @description Maps secured Fulfillment lifecycle HTTP operations to the Fulfillment facade.
 * @layer controller
 * @owner fulfillment
 * @override Later modules may adapt request mapping while preserving Fulfillment-owned permissions, validation, and secret-redaction boundaries.
 */
module.exports = {
  ...
  executeFacade: function (operation, request, callback) {
    ...
    let promise = Promise.resolve().then(() =>
      FACADE.DefaultFulfillmentOperationsLifecycleFacade[operation](input),
    );
    if (callback)
      return promise.then((result) => callback(null, result)).catch(callback);
    return promise;
  },
  /** Executes one Fulfillment lifecycle action. */
